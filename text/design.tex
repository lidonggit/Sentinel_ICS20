...
\textcolor{dong2}{because they are used sparsely and periodically}. %and short-lived tensors (if the space of fast memory is not enough to accommodate the upcoming data migration). 
...
The data migration is controlled by the \textit{migration interval} \textcolor{dong2}{which is a system parameter that} determines how frequently we migrate data between fast and slow memories. %%%A training step is partitioned into many equal-sized migration intervals. Figure~\ref{fig:migration_interval} generally depicts data migration.
